...
const ExcelUpload = ({ postUrl, expectedHeaders = [] }) => {
  ...
  const processFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.length === 0) throw new Error("File kosong atau tidak memiliki data.");
  ...
  const handleUpload = async () => {
    if (expectedHeaders.length > 0) {
      const validation = validateHeaders(excelData[0], expectedHeaders);
      if (validation.errors.length > 0) {
        setHeaderValidation(validation.errors);
        setToast({ text: validation.error, type: "error" });
        return;
      }
    }

    try {
      const { success, error } = await uploadFile(excelData);
      if (success) {
        setToast({ type: "success", text: "File berhasil diunggah." });
        setIsModalOpen(false);
        resetFileInput();
      } else {
        throw new Error(error || "Error saat mengimport file");
      }
    } catch (err) {
                  ...
                  <thead>
                    <tr>
                      {excelData[0].map((receivedHeader, index) => {
                        ...
                        return (
                          <th key={index} className={`border px-4 py-2 ${error ? "bg-red-100 text-red-600 border-red-400" : ""}`}>
